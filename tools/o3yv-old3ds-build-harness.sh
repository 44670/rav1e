...
usage() {
  cat <<'USAGE'
usage: tools/o3yv-old3ds-build-harness.sh [representative.o3yv] [iterations] [target_us]
...
tools/o3yv-old3ds-prepare-stream.sh "$input" old3ds/generated "$iterations"
...
make -C old3ds RUST_LIB="$(pwd)/$rust_lib" O3YV_TARGET_US="$target_us"
